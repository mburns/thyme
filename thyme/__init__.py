...
class Person(db.Model):
    ...
    # Relationships
    awards = db.relationship("Award", secondary=person_awards, back_populates="recipients")
        ...
        if self.birth_date:
            events.append({
                ...
                "text": {
                    ...
                    "text": f"Birth of {self.name}" + (f", {self.occupation}" if self.occupation else ""),
                },
                "media": {
        ...
        if self.death_date:
            events.append({
                ...
                "text": {
                    ...
                    "text": f"Death of {self.name}" + (f", {self.occupation}" if self.occupation else ""),
                },
                "media": {
                    "url": self.image_url or "",
                    "caption": self.name,
...
class Award(db.Model):
    ...
    # Relationships
    recipients = db.relationship("Person", secondary=person_awards, back_populates="awards")
    ...
    def to_timeline_format(self) -> Dict[str, Any]:
        ...
        for person in self.recipients:
            ...
            award_date = db.session.query(person_awards.c.award_date).filter_by(
                person_id=person.id, award_id=self.id
            ).scalar()
            ...
            if award_date:
                events.append({
                    ...
                    "text": {
                        ...
                        "text": f"{person.name} received {self.name}" + (f" for {self.description}" if self.description else ""),
                    },
                    "media": {
                        "url": person.image_url or self.image_url or "",
                        "caption": person.name,
                        "credit": "Wikimedia Commons",
                    } if (person.image_url or self.image_url) else None,
                    "group": "Awards",
...
class War(db.Model):
    ...
    # Relationships
    participants = db.relationship("Person", secondary=person_wars, back_populates="wars")
        ...
        if self.start_date:
            events.append({
                ...
                "text": {
                    ...
                    "text": f"Start of {self.name}" + (f", {self.description}" if self.description else ""),
                },
                "media": {
                    "url": self.image_url or "",
                    "caption": self.name,
    ...
    def to_timeline_format(self) -> Dict[str, Any]:
        ...
        if self.event_date:
            events.append({
                ...
                "text": {
                    "headline": self.name,
                    "text": f"{self.name}" + (f", {self.description}" if self.description else ""),
                },
                "media": {
                    "url": self.image_url or "",
                    "caption": self.name,
