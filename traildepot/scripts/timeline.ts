  ...
  jsonHandler(async (req): Promise<TimelineResponse> => {
    ...
    try {
      ...
      if (includeTitles) {
        ...
        const titleEvents = (titlesData.records || [])
          .map((title: any) => {
      ...
      if (includePersons) {
        ...
        const personEvents = (personsData.records || [])
          .map((person: any) => {
            if (!person.birthYear || !person.primaryName) return null;

            const profession = person.primaryProfession
              ? person.primaryProfession.split(",")[0].trim()
              : "Unknown";
            ...
            return {
              ...
              end_date: person.deathYear
                ...
                : undefined,
              text: {
                headline: person.deathYear
                  ? `${person.primaryName} (${person.birthYear}-${person.deathYear})`
                  : `${person.primaryName} (Born ${person.birthYear})`,
                text: `<p><strong>Birth:</strong> ${person.birthYear}${person.birthMonth ? `-${person.birthMonth}` : ""}${person.birthDay ? `-${person.birthDay}` : ""}</p>
                     ${
                       person.deathYear
                         ? `<p><strong>Death:</strong> ${person.deathYear}${person.deathMonth ? `-${person.deathMonth}` : ""}${person.deathDay ? `-${person.deathDay}` : ""}</p>
      ...
      timelineEvents.sort((a, b) => {
        const dateA = new Date(
          a.start_date.year,
          (a.start_date.month || 1) - 1,
          a.start_date.day || 1,
        );
        const dateB = new Date(
          b.start_date.year,
          (b.start_date.month || 1) - 1,
          b.start_date.day || 1,
        );
