...
ALTER TABLE persons
ADD CONSTRAINT check_birth_death_years CHECK (
        birth_year IS NULL
        OR death_year IS NULL
        OR birth_year <= death_year
    );
ALTER TABLE titles
ADD CONSTRAINT check_start_end_years CHECK (
        start_year IS NULL
        OR end_year IS NULL
        OR start_year <= end_year
    );
ALTER TABLE titles
ADD CONSTRAINT check_episode_fields CHECK (
        (
...
CREATE OR REPLACE FUNCTION update_all_stats() RETURNS VOID AS $$ BEGIN
UPDATE imdb_stats
SET stat_value = (
        SELECT COUNT(*)
        FROM persons
...
SET stat_value = (
    ...
    last_updated = UNIXEPOCH()
WHERE stat_name = 'total_principals';
UPDATE imdb_stats
SET stat_value = (
        SELECT COUNT(*)
        FROM title_akas
...
SET stat_value = (
    ...
    last_updated = UNIXEPOCH()
WHERE stat_name = 'total_crew';
END;
$$ LANGUAGE plpgsql;
-- 12. Add additional constraints for data quality
ALTER TABLE title_principals
ADD CONSTRAINT check_ordering_positive CHECK (ordering > 0);
ALTER TABLE title_akas
ADD CONSTRAINT check_ordering_positive CHECK (ordering > 0);
ALTER TABLE ratings
ADD CONSTRAINT check_rating_range CHECK (
        average_rating >= 0.0
        AND average_rating <= 10.0
    );
-- 13. Add indexes for text search (if full-text search is available)
-- Note: This would require FTS5 extension in SQLite
-- CREATE VIRTUAL TABLE titles_fts USING fts5(primary_title, original_title, content='titles', content_rowid='id');
...
CREATE OR REPLACE FUNCTION get_person_filmography(person_nconst TEXT) RETURNS TABLE (
        tconst TEXT,
        primary_title TEXT,
        title_type TEXT,
        start_year INTEGER,
