...
Usage:
    python scripts/import_imdb_to_traildepot.py [--data-dir data/] [--batch-size 1000]
"""
...
class IMDBImporter:
    ...
    def _insert_persons_batch(self, batch: List[tuple]):
        ...
        try:
            self.cursor.executemany("""
                INSERT INTO persons (id, nconst, primary_name, birth_year, death_year, primary_profession, known_for_titles)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, batch)
            self.conn.commit()
            
            # Update cache
            for person_id, nconst, _, _, _, _, _ in batch:
                self.person_cache[nconst] = person_id
    ...
    def _insert_titles_batch(self, batch: List[tuple]):
        ...
        try:
            self.cursor.executemany("""
                INSERT INTO titles (id, tconst, title_type, primary_title, original_title, is_adult, 
            ...
            # Update cache
            for title_id, tconst, _, _, _, _, _, _, _, _, _, _, _ in batch:
                self.title_cache[tconst] = title_id
        ...
        with gzip.open(file_path, 'rt', encoding='utf-8') as f:
            ...
            for line_num, line in enumerate(f, 2):
                try:
                    ...
                    batch.append((self.get_uuid(), title_id, rating_float, votes_int))
    ...
    def _insert_ratings_batch(self, batch: List[tuple]):
        ...
        try:
            self.cursor.executemany("""
                INSERT INTO ratings (id, title_id, average_rating, num_votes)
                VALUES (?, ?, ?, ?)
            """, batch)
            self.conn.commit()
        ...
        with gzip.open(file_path, 'rt', encoding='utf-8') as f:
            ...
            for line_num, line in enumerate(f, 2):
                try:
                    ...
                    (title_id_str, ordering, title, region, language, types, 
                     attributes, is_original_title) = fields
                    
                    # Get title ID
                    title_uuid = self.title_cache.get(title_id_str)
                    if not title_uuid:
                        logger.warning(f"Line {line_num}: Title not found: {title_id_str}")
                        continue
    ...
    def _insert_akas_batch(self, batch: List[tuple]):
        ...
        try:
            self.cursor.executemany("""
                INSERT INTO title_akas (id, title_id, ordering, title, region, language, types, attributes, is_original_title)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, batch)
            self.conn.commit()
        ...
        with gzip.open(file_path, 'rt', encoding='utf-8') as f:
            ...
            for line_num, line in enumerate(f, 2):
                try:
                    ...
                    batch.append((self.get_uuid(), title_uuid, directors_json, writers_json))
    ...
    def _insert_crew_batch(self, batch: List[tuple]):
        ...
        try:
            self.cursor.executemany("""
                INSERT INTO title_crew (id, title_id, directors, writers)
                VALUES (?, ?, ?, ?)
            """, batch)
            self.conn.commit()
    ...
    def _insert_principals_batch(self, batch: List[tuple]):
        ...
        try:
            self.cursor.executemany("""
                INSERT INTO title_principals (id, title_id, person_id, ordering, category, job, characters)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, batch)
            self.conn.commit()
    ...
    def run_import(self, data_dir: str):
        ...
        try:
            ...
            # Pre-cache existing persons and titles
            logger.info("Caching existing persons...")
            self.cursor.execute("SELECT id, nconst FROM persons")
            for person_id, nconst in self.cursor.fetchall():
                self.person_cache[nconst] = person_id
            logger.info(f"Cached {len(self.person_cache)} persons.")

            logger.info("Caching existing titles...")
            self.cursor.execute("SELECT id, tconst FROM titles")
            for title_id, tconst in self.cursor.fetchall():
                self.title_cache[tconst] = title_id
            logger.info(f"Cached {len(self.title_cache)} titles.")
...
def main():
    ...
    importer = IMDBImporter(db_path, args.batch_size)
